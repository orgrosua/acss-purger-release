...
namespace _YabeAcssPurger\EDD_SL;
...
class EDD_SL_Plugin_Updater
{
    ...
    public function show_update_notification($file, $plugin)
    {
        ...
        \printf('<tr class="plugin-update-tr %3$s" id="%1$s-update" data-slug="%1$s" data-plugin="%2$s">', $this->slug, $file, \in_array($this->name, $this->get_active_plugins(), \true) ? 'active' : 'inactive');
        echo '<td colspan="3" class="plugin-update colspanchange">';
        echo '<div class="update-message notice inline notice-warning notice-alt"><p>';
        $changelog_link = '';
        if (!empty($update_cache->response[$this->name]->sections->changelog)) {
            $changelog_link = \add_query_arg(array('edd_sl_action' => 'view_plugin_changelog', 'plugin' => \urlencode($this->name), 'slug' => \urlencode($this->slug), 'TB_iframe' => 'true', 'width' => 77, 'height' => 911), \self_admin_url('index.php'));
        }
        $update_link = \add_query_arg(array('action' => 'upgrade-plugin', 'plugin' => \urlencode($this->name)), \self_admin_url('update.php'));
        \printf(
    ...
    public function plugins_api_filter($_data, $_action = '', $_args = null)
    {
        ...
        $to_send = array('slug' => $this->slug, 'is_ssl' => \is_ssl(), 'fields' => array('banners' => array(), 'reviews' => \false, 'icons' => array()));
    ...
    private function api_request($_action, $_data)
    {
        ...
        if (\trailingslashit(\home_url()) === $this->api_url) {
            return \false;
        }
    ...
    private function get_version_from_remote()
    {
        ...
        $request = \wp_remote_post($this->api_url, array('timeout' => 15, 'sslverify' => $this->verify_ssl(), 'body' => $api_params));
    ...
    public function set_version_info_cache($value = '', $cache_key = '')
    {
        ...
        $data = array('timeout' => \strtotime('+3 hours', \time()), 'value' => \wp_json_encode($value));
